...
# Json -> autogenerated modifiable pyside gui layout (with save button)
class JsonLayout(QGridLayout):
    def __init__(self, parent_tab=None):
        super().__init__(parent_tab)
        self.gui_data = []

        cur_data = data_utils.MainSettings().get_data()
        cur_row = 0
        cur_col = 0
        for k, v in cur_data.items():
            if isinstance(v, bool):
                cb_temp = QCheckBox(parent_tab)
                cb_temp.setText(str(k))
                cb_temp.setChecked(v)
                self.gui_data.append((k, type(v), cb_temp))
                self.addWidget(cb_temp, cur_row, cur_col)
            elif isinstance(v, int):
                int_validator = QIntValidator()
                int_validator.setRange(0, 10000)
                line_edit = QLineEdit(parent_tab)
                line_edit.setValidator(int_validator)
                line_edit.setText(str(v))

                line_label = QLabel(parent_tab)
                line_label.setText(k)

                self.gui_data.append((k, type(v), line_edit))
                self.addWidget(line_label, cur_row, cur_col)
                self.addWidget(line_edit, cur_row, cur_col+1)
            else:
                print("JsonLayout: Unsupported json type: " + str(type(v)))
            cur_row += 1

        self.btn_temp = QPushButton(parent_tab)
        self.btn_temp.setText("Save Settings")
        self.btn_temp.clicked.connect(self.btn_clicked)
        self.addWidget(self.btn_temp, cur_row, cur_col)

        parent_tab.setLayout(self)

    def save_data(self):
        updated_data = {}
        for n, t, v in self.gui_data:  # name, type, value [gui element]
            if isinstance(v, QCheckBox):
                data_val = v.isChecked()
                updated_data[n] = data_val
            elif isinstance(v, QLineEdit):
                data_val = t(v.text())
                updated_data[n] = data_val

        data_utils.MainSettings().save_data(updated_data)
